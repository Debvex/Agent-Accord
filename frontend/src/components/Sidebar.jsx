...
export default function Sidebar({
  ...
  return (
    <aside className="relative h-full w-[320px] shrink-0 overflow-hidden border-r border-slate-800/80 bg-slate-950/90 text-slate-100 shadow-[8px_0_40px_rgba(2,6,23,0.5)] backdrop-blur-md">
      ...
      <div className="relative z-10 flex h-full flex-col">
        <header className="flex items-start justify-between border-b border-slate-800/80 bg-slate-950/60 px-4 py-3">
          <div className="space-y-1">
            <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-200">
              <Sparkles className="h-3.5 w-3.5 text-rose-400" />
              Dialogue Stream
            </div>
            <p className="text-[10px] uppercase tracking-[0.22em] text-slate-400">Agent Accord</p>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-end gap-2 rounded-xl border border-slate-800 bg-slate-900/80 px-3 py-3 shadow-inner">
              ...
              <button
                ...
                onClick={onRun}
                disabled={isRunning}
                className="flex h-9 w-9 items-center justify-center rounded-lg border border-amber-500/40 bg-amber-500/20 text-amber-300 transition-colors hover:bg-amber-500/30 hover:text-amber-200 disabled:opacity-50"
            ...
            <div className="flex flex-wrap gap-1">
              {PRESET_MANDATES.map((preset, index) => (
                <button
                  ...
                  className="max-w-full truncate rounded-full border border-slate-800 bg-slate-900/60 px-2.5 py-1 text-left text-[10px] text-slate-400 transition-colors hover:border-slate-700 hover:text-slate-200 disabled:opacity-50"
                >
